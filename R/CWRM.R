...
# Routines for fitting CWRM -----------------------------------------------

# This file includes the original routines of
...
calcobj_i <- function (X,Y, iter, pa)   ### it calculates the objective function
{
  ...
  for (k in 1:pa$K) {
    w=iter$cw[k]*mnormt::dmnorm(X,iter$center[k,],ssclmat(iter$sigma,k)) * stats::dnorm(Y,cbind(one,X)%*%cbind(iter$b[k,]),sd=sqrt(iter$v[k]))
    w=w*(w>=0)
    ww=w+ww
  }

  iter$obj <- sum(log(ww[iter$assig>0]))
...
findClustAssig_i <- function (X,Y, iter, pa)  ### it obtains the assigment corresponding to each point to the K populations and the set of trimmed points
{
  ...
  for (k in 1:pa$K)
    ll[,k] <- iter$cw[k]*mnormt::dmnorm(X,iter$center[k,],ssclmat (iter$sigma,k)) * stats::dnorm(Y,cbind(one,X)%*%cbind(iter$b[k,]),sd=sqrt(iter$v[k]))
  ll <- ll*(ll>=0)
  ...
  iter$z_ij=ll/(pre.z_ + (pre.z_ ==0))+(pre.z ==0)%*%matrix(rep(0,pa$K),nrow=1)*t(stats::rmultinom(pa$n, 1, iter$cw))
...
dmnorm <- function (X,mu,sigma) ((2 * pi)^(-length(mu) / 2)) * (det(sigma)^(-1/ 2)) * exp (-0.5 * stats::mahalanobis (X, mu, sigma)) ### multivariate normal density function
...
InitClusters_i <- function (X, Y, iter, pa)  ### it obtains random initial solution
{
  ...
  for (k in 1:pa$K)
  {
    ...
    X.ini = X [drop = F,idx,]
    cc <- (pa$init/(pa$init+1))*stats::cov(X.ini)
    iter$sigma[,,k] <- cc
    ...
    iter$b[k,] <- t(MASS::ginv(t(X.ini)%*%X.ini)%*%t(X.ini)%*%Y.ini)
    #iter$b[k,] <- t(Ginv(t(X.ini)%*%X.ini)$Ginv%*%t(X.ini)%*%Y.ini)
    # after changing from idbreg to matlib, Ginv gives back the matrix with subsetting with $Ginv
    e=Y.ini-X.ini%*%cbind(iter$b[k,])
    iter$v[k] <- (pa$init/(pa$init+1))*stats::cov(e)
...
estimClustPar_i <- function (X, Y, iter, pa) ### it obtains estimations for the model parameters
{
  ...
  for (k in 1:pa$K)
  {
    if (iter$csize[k] > pa$zero.tol)
    {
      ...
      one=matrix(1,nrow=pa$n,ncol=1)
      X_=cbind(one,X)
      iter$b[k,] <- t(MASS::ginv(  t(X_*iter$z_ij[,k])%*%X_  )%*%t(X_*iter$z_ij[,k])%*%Y)
      # after changing to matlib      iter$b[k,] <- t(Ginv(  t(X_*iter$z_ij[,k])%*%X_  )$Ginv%*%t(X_*iter$z_ij[,k])%*%Y)
