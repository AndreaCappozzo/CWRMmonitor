...
#'
#' @examples
Discrim_Function_CWM <- function(y, X, alpha, assig, tau, beta, mu, Sigma, sigma) {
  ...
  max_G_regr <- apply(log_comp_regr, 1, which.max)
  d_i_regr <- apply(log_comp_regr, 1, max)

  # DF_regr <- # Original formula in \cite{Garcia-Escudero2011}
  #   d_i_regr - sapply(1:N, function(i)
  #     max(log_comp_regr[i, -max_G_regr[i]]))
  DF_regr <- # Formula in \cite{Fritz2012}, as implementedi in tclust software
    sapply(1:N, function(i)
      max(log_comp_regr[i, -max_G_regr[i]])) - d_i_regr
  ...
  max_G_X <- apply(log_comp_X, 1, which.max)
  d_i_X <- apply(log_comp_X, 1, max)
  DF_X <-
    sapply(1:N, function(i)
      max(log_comp_X[i, -max_G_X[i]])) - d_i_X
  ...
  max_G_CWM <- apply(log_comp_CWM, 1, which.max)
  d_i_CWM <- apply(log_comp_CWM, 1, max)
  DF_CWM <-
    sapply(1:N, function(i)
      max(log_comp_CWM[i, -max_G_CWM[i]])) - d_i_CWM
  # DF_CWM <-
  #   d_i_CWM - sapply(1:N, function(i)
  #     max(log_comp_CWM[i, -max_G_CWM[i]]))

  # If trimming is present, I compute the DF according to pag 590 second column \cite{Garcia-Escudero2011}
  # For regr, X and total

  if(alpha>0){
    ind_trimmed_units <- which(assig==0)

    d_alpha_regr <- (sort(d_i_regr))[ceiling(alpha*N)+1] # formula end of page 18 \cite{Fritz2012}
    d_alpha_X <- (sort(d_i_X))[ceiling(alpha*N)+1]
    d_alpha_CWM <- (sort(d_i_CWM))[ceiling(alpha*N)+1]

    DF_regr[ind_trimmed_units] <- d_i_regr[ind_trimmed_units]-d_alpha_regr
    DF_X[ind_trimmed_units] <- d_i_X[ind_trimmed_units]-d_alpha_X
    DF_CWM[ind_trimmed_units] <- d_i_CWM[ind_trimmed_units]-d_alpha_CWM
  }
...
#'
#' @param X
#' @param group_representative_fake_obs
#' @param old_partition
#' @param new_partition
#' @param new_model
#' @param old_model
...
plausible_solution_finder <- function(solutions_df, L, epsilon_ARI){ # L is the number of plausible solutions to retain
  ...
  while(criterion){
    ...
    plausible_solutions_df <- dplyr::bind_rows(plausible_solutions_df,dplyr::filter(similar_partitions_df, is_best))
    plausible_solutions_df
    remaining_solutions_df = dplyr::anti_join(
...
solutions_ranking_identifier <-
  function(solutions_df,
    ...
    df_optimal_solutions <-
      df_plausible_solutions %>%
      dplyr::slice(optimal_solution_finder(df_plausible_solutions = df_plausible_solutions, epsilon_ARI = epsilon_ARI, threshold_n_sporious=threshold_n_sporious)) %>%
      tibble::rowid_to_column(var = "rank_best") %>%
      dplyr::select(-stable_solution, -n_solution,-is_best)

    df_spurious_sol <-
      dplyr::anti_join(df_plausible_solutions, df_optimal_solutions)
    ...
    df_4_plot_cars <-  solutions_df %>%
      ...
      dplyr::left_join(dplyr::select(df_optimal_solutions,1:6)) %>%
      dplyr::mutate(dplyr::across(c(restr_factor_X, restr_factor_Y), .fns = factor))

    for(rank_best_mod in 1:nrow(df_optimal_solutions)) {

      current_optimal_solution <-
        dplyr::slice(df_optimal_solutions, rank_best_mod)
